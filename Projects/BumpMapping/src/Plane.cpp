...
void Plane::createPlane(int side)
{
	...
	glBufferData(GL_ARRAY_BUFFER, getVertexSizeInBytes(), &plane[0], GL_STATIC_DRAW);
	...
	// Pass texture coordinate array
	glBufferData(GL_ARRAY_BUFFER, getTextureCoordsSizeInBytes(), &textureCoords[0], GL_STATIC_DRAW);
	glVertexAttribPointer(1, 2, GL_FLOAT, GL_FALSE, 0, 0); //location 1: textures
